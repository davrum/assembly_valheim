...
public class GraphicsSettings : SettingsBase
{
	...
	public override void FixBackButtonNavigation(Button backButton)
	{
		...
		{
			m_antialiasingToggle, m_softPartToggle, m_sunShaftsToggle, m_chromaticAberrationToggle, m_motionBlurToggle, m_depthOfFieldToggle, m_bloomToggle, m_ssaoToggle, m_tesselationToggle, m_distantShadowsToggle,
			m_renderScaleSlider, m_pointLightsSlider, m_shadowQualitySlider, m_lightsSlider, m_levelOfDetailSlider, m_vegetationSlider, m_graphicPresetLeft, m_vsyncToggle, m_fpsLimitSlider, m_resolutionDropdown
	...
	private void Update()
	{
		...
		if ((bool)m_resolutionDropdownScrollRectEnsureVisible && (bool)currentSelectedGameObject && ZInput.GamepadActive)
		{
			m_resolutionDropdownScrollRectEnsureVisible.CenterOnItem(currentSelectedGameObject.GetComponent<RectTransform>());
		}
	}
	...
	private void OnResolutionDropdownExpanded(bool expanded)
	{
		...
		if (m_resolutionDropdownScrollRectEnsureVisible == null && expanded)
		{
			FindResolutionScrollRect();
		}
	}
	...
	private void FindResolutionScrollRect()
	{
		...
		{
			m_resolutionDropdownScrollRectEnsureVisible = componentsInChildren[0];
		}
		else
		{
	...
	private void FillResList()
	{
		...
		foreach (Resolution resolution in m_resolutions)
		{
			...
			if (Screen.fullScreenMode == FullScreenMode.ExclusiveFullScreen || !list.Contains(text))
			{
				...
				{
					list.Add($"{text} {Mathf.Round((float)resolution.refreshRateRatio.value)}hz");
				}
				else
				{
					list.Add(text);
	...
	public void OnTestResolution()
	{
		...
		if (m_resSwitchDialog.transform.parent == base.transform)
		{
			m_resSwitchDialog.transform.parent = m_resSwitchDialog.transform.parent.parent;
		}
		m_resSwitchDialog.GetComponent<ResolutionSwitchDialogTimedRemoval>().ResCountdownTimer = 5f;
