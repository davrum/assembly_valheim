...
public class ZSteamSocketOLD : IDisposable, ISocket
{
	...
	private static void RegisterGlobalCallbacks()
	{
		if (m_connectionFailed == null)
		{
			...
			m_connectionFailed = Callback<P2PSessionConnectFail_t>.CreateGameServer(OnConnectionFailed);
		}
		if (m_SessionRequest == null)
		{
			m_SessionRequest = Callback<P2PSessionRequest_t>.CreateGameServer(OnSessionRequest);
		}
	}
	...
	public void Close()
	{
		...
		if (m_peerID != CSteamID.Nil)
		{
			...
			Thread.Sleep(100);
			SteamGameServerNetworking.GetP2PSessionState(m_peerID, out var pConnectionState);
			ZLog.Log("  P2P state, bytes in send queue:" + pConnectionState.m_nBytesQueuedForSend);
			SteamGameServerNetworking.CloseP2PSessionWithUser(m_peerID);
			SteamGameServer.EndAuthSession(m_peerID);
	...
	private void SendQueuedPackages()
	{
		...
		while (m_sendQueue.Count > 0)
		{
			...
			if (SteamGameServerNetworking.SendP2PPacket(m_peerID, array, (uint)array.Length, eP2PSendType))
			{
	...
	private static void ReceivePackages()
	{
		...
		while (SteamGameServerNetworking.IsP2PPacketAvailable(out pcubMsgSize))
		{
			...
			if (SteamGameServerNetworking.ReadP2PPacket(array, pcubMsgSize, out var _, out var psteamIDRemote))
			{
