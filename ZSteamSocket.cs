...
public class ZSteamSocket : IDisposable, ISocket
{
	...
	public ZSteamSocket(CSteamID peerID)
	{
		...
		m_con = SteamGameServerNetworkingSockets.ConnectP2P(ref m_peerID, 0, 0, null);
	...
	public ZSteamSocket(HSteamNetConnection con)
	{
		RegisterGlobalCallbacks();
		m_con = con;
		SteamGameServerNetworkingSockets.GetConnectionInfo(m_con, out var pInfo);
	...
	private static void RegisterGlobalCallbacks()
	{
		...
		{
			m_statusChanged = Callback<SteamNetConnectionStatusChangedCallback_t>.CreateGameServer(OnStatusChanged);
	...
	private static void OnStatusChanged(SteamNetConnectionStatusChangedCallback_t data)
	{
		...
		if (data.m_info.m_eState == ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connected && data.m_eOldState == ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connecting)
		{
			...
			{
				if (SteamGameServerNetworkingSockets.GetConnectionInfo(data.m_hConn, out var pInfo))
				{
	...
	public void Close()
	{
		if (m_con != HSteamNetConnection.Invalid)
		{
			...
			SteamGameServerNetworkingSockets.CloseConnection(m_con, 0, "", bEnableLinger: false);
	...
	public bool StartHost()
	{
		...
		{
			m_port = (ushort)m_steamDataPort
		};
		m_listenSocket = SteamGameServerNetworkingSockets.CreateListenSocketIP(ref localAddress, 0, null);
		m_hostSocket = this;
	...
	public bool Flush()
	{
		SendQueuedPackages();
		_ = m_con;
		SteamGameServerNetworkingSockets.FlushMessagesOnConnection(m_con);
	...
	private void SendQueuedPackages()
	{
		...
		while (m_sendQueue.Count > 0)
		{
			...
			EResult eResult = SteamGameServerNetworkingSockets.SendMessageToConnection(m_con, intPtr, (uint)array.Length, 8, out pOutMessageNumber);
	...
	public ZPackage Recv()
	{
		...
		if (SteamGameServerNetworkingSockets.ReceiveMessagesOnConnection(m_con, array, 1) == 1)
		{
	...
	public int GetSendQueueSize()
	{
		...
		if (SteamGameServerNetworkingSockets.GetConnectionRealTimeStatus(m_con, ref pStatus, 0, ref pLanes) == EResult.k_EResultOK)
		{
